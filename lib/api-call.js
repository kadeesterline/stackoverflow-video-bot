...
export async function makeApiCall(questionURL, questionDataObj) {
  ...
  const questionId = url
    .match(/\/(\d+)+[\/]?/g)
    .map((id) => id.replace(/\//g, ""));
  const qURL = `https://api.stackexchange.com/2.3/questions/${questionId}?order=desc&sort=activity&site=stackoverflow`;
  const aURL = `https://api.stackexchange.com/2.3/questions/${questionId}/answers?order=desc&sort=activity&site=stackoverflow`;

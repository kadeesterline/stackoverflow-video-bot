...
export async function screenshot(questionURL, questionDataObj, fileNames) {
  const url = questionURL;
  // let tempArray = ["question-title", "question-body"];
  /**
  ...
  // async function spreadName(num) {
  //   tempArray = [...tempArray, `answer${num}`];
  // }
  ...
  const getScreenShot = async () => {
    ...
    for (let i = 0; i < questionDataObj.answerIds.length; i++) {
      // await spreadName(questionDataObj.answerIds[i]);
